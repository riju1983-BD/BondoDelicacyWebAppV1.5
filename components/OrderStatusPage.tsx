        ...
        {steps.map((step, index) => (
          <React.Fragment key={step}>
            <div className="flex flex-col items-center flex-shrink-0 w-24 md:flex-1">
              ...
              <p
                className={`mt-2 text-xs sm:text-sm text-center font-semibold whitespace-nowrap ${index <= currentIndex ? "text-white" : "text-gray-400"
                  }`}
              >
                {step}
              </p>
            ...
            {index < steps.length - 1 && (
              <div
                className={`flex-shrink-0 w-12 md:flex-1 h-1 mt-4 ${index < currentIndex ? "bg-green-500" : "bg-gray-600"
                  }`}
              ></div>
            )}
          </React.Fragment>
