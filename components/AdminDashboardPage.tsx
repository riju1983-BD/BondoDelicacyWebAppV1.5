...
import { apiGetLoyaltyConfig, apiSetLoyaltyConfig, apiGetLiveMenu, apiUpdateItemAvailability, apiGetAllBrands, apiGetComplaints, apiProcessRefundApproval, apiRejectComplaint, apiGetAllActiveReservations, apiUpdateReservation, apiGetAdminCategoriesMenu, apiGetOrders } from '../services/apiService';
...
const AdminDashboardPage: React.FC = () => {
    const [activeTab, setActiveTab] = useState<'menu' | 'orders' | 'loyalty' | 'complaints' | 'reservations'>('menu');
    ...
    const fetchOrders = useCallback(async () => {
        ...
        try { const allOrders = await apiGetOrders(); setOrders(allOrders); }
        catch (err) { console.error("Failed to fetch complaints:", err); }
...
}, [activeTab, selectedBrandId, fetchMenu, fetchOrders, fetchComplaints, fetchReservations]);
    ...
    <div className="min-h-screen bg-gray-800 text-white p-4 sm:p-6 lg:p-8 flex items-center justify-center">
        <div className="w-full max-w-6xl bg-gray-900 rounded-lg shadow-2xl p-8">
            ...
            <div className="flex border-b border-gray-700 mb-6 overflow-x-auto">
                ...
                <button onClick={() => setActiveTab('orders')} className={`flex-shrink-0 py-2 px-4 font-semibold ${activeTab === 'orders' ? 'border-b-2 border-cyan-400 text-cyan-400' : 'text-gray-400'}`}>Orders</button>
            ...
            {activeTab === 'orders' && (
                <div className="animate-fade-in">
                    {isLoadingOrders ? <div className="flex justify-center p-8"><Spinner className="w-8 h-8" /></div> :
                        orders.length === 0 ? <p className="text-center text-gray-500 py-8">No active Orders.</p> :
                                        ...
                                        {orders.map(o => (
                                            <tr key={o.id}>
                                                ...
                                                <td className="px-4 py-4 whitespace-nowrap">
                                                    <div className="text-sm font-bold text-cyan-400 font-mono">{o.id}</div>
                                                    <div className="text-xs text-gray-400">{brandsData[o.brandId]?.name}</div>
                                                ...
                                                <td className="px-4 py-4">
                                                    <div className="text-sm font-medium text-white">{o.customer.name}</div>
                                                    <div className="text-sm text-gray-400">{o.customer.phone}</div>
                                                    <div className="text-xs text-gray-500">{o.customer.email}</div>
                                                </td>

                                                {/* DATE & TIME */}
                                                <td className="px-4 py-4 whitespace-nowrap">
                                                    <div className="text-sm text-white">{new Date(o.createdAt).toLocaleDateString()}</div>
                                                    <div className="text-xs text-gray-400">{new Date(o.createdAt).toLocaleTimeString()}</div>
                                                </td>
                                                ...
                                                <td className="px-4 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                    <div className="flex justify-end gap-2">
                                                        <button
                                                            className="bg-green-600 hover:bg-green-500 text-white px-3 py-1.5 rounded-md text-xs font-bold uppercase"
                                                        >
                                                            Update
                                                        </button>
                                                    </div>
